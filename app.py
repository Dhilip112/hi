...
async def upload_and_query(query: str = Form(...), image: UploadFile = File(None)):
    try:
        ...
        # Make request to the single model
        response = make_api_request("meta-llama/llama-4-scout-17b-16e-instruct")
        ...
        if response.status_code == 200:
            ...
            raise HTTPException(status_code=response.status_code, detail=f"Error from API: {response.status_code}")
